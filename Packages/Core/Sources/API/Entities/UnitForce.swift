...
class UnitForce: Dimension, @unchecked Sendable {

    /// The SI base unit of force.
    /// Defined as the force required to accelerate 1 kilogram by 1 meter per second squared.
    /// `1 N = 1 kg·m/s²`
    static let newton = UnitForce(symbol: "N", converter: UnitConverterLinear(coefficient: 1))

    /// Kilonewton — commonly used in structural engineering and mechanical loads.
    /// `1 kN = 1,000 N`
    static let kiloNewton = UnitForce(symbol: "kN", converter: UnitConverterLinear(coefficient: 1_000))

    /// Meganewton — used in heavy industries, aerospace, and large-scale engineering.
    /// `1 MN = 1,000,000 N`
    static let megaNewton = UnitForce(symbol: "MN", converter: UnitConverterLinear(coefficient: 1_000_000))

    /// Dyne — a CGS unit (centimeter-gram-second system).
    /// Often used in theoretical physics or legacy scientific literature.
    /// `1 dyn = 1e-5 N`
    static let dyne = UnitForce(symbol: "dyn", converter: UnitConverterLinear(coefficient: 1e-5))
    ...
    /// `1 kgf = 9.80665 N`
    static let kilogramForce = UnitForce(symbol: "kgf", converter: UnitConverterLinear(coefficient: 9.80665))

    /// Gram-force (gf) — small-scale gravitational force unit.
    /// `1 gf = 0.00980665 N`
    static let gramForce = UnitForce(symbol: "gf", converter: UnitConverterLinear(coefficient: 0.00980665))

    /// Metric ton-force (tf) — force of gravity on a mass of one metric ton.
    /// `1 tf = 9806.65 N`
    static let tonForce = UnitForce(symbol: "tf", converter: UnitConverterLinear(coefficient: 9806.65))
    ...
    /// `1 lbf = 4.44822 N`
    static let poundForce = UnitForce(symbol: "lbf", converter: UnitConverterLinear(coefficient: 4.44822))

    /// Ounce-force (ozf) — used for smaller-scale force in imperial systems.
    /// `1 ozf = 0.2780139 N`
    static let ounceForce = UnitForce(symbol: "ozf", converter: UnitConverterLinear(coefficient: 0.2780139))
